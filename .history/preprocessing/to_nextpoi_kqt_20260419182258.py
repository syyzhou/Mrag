...
def main():
    ...
    qa_pairs_test = generate_qa_pairs(
        ...
        train_history_dict=train_traj_dict, # 修改为 None，不补充训练集历史
        mode='test', 
        args=args
    )
    ...
    with open(f'{path}train_qa_pairs_kqt_30.json', 'w') as f:
        json.dump(qa_dict_train, f, indent=2)

    print(f"Saving {len(qa_pairs_test)} test samples...")
    with open(f'{path}test_qa_pairs_kqt_30.txt', 'w') as f:

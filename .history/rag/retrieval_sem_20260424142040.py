...
def evaluate_acc(retrieved, answer_poi_id, all_pois, top_k, per_center_hops):
    """
    计算acc@1和acc@5
    - acc@1: 正确答案是否在检索结果的第一位（center POI或第一跳）
    - acc@5: 正确答案是否在检索结果的前5位
    
    检索结果顺序：
    - 首先是top_k个center POI（按排名）
    - 然后是每个center POI的per_center_hops个下一跳
    """
    if answer_poi_id is None:
    ...
    # 首先添加center POI（按排名顺序）
    for item in retrieved[:top_k]:
        candidates.append(item["center_poi"])
    
    # 然后添加每个center的下一跳（按center的排名，然后按跳数排序）
    for item in retrieved[:top_k]:
        next_hops = item["next_hops"]
        # 按跳数排序（次数多的优先）
        sorted_hops = sorted(next_hops.items(), key=lambda x: -x[1])
...
def process_qa_file(
    ...
    if valid_eval > 0:
        ...
        # 打印acc@1和acc@5
        acc1_rate = acc_counts["acc@1"] / valid_eval
        acc5_rate = acc_counts["acc@5"] / valid_eval
        print(f"  {split_name} acc@1: {acc_counts['acc@1']} / {valid_eval} = {acc1_rate:.4f}")
        print(f"  {split_name} acc@5: {acc_counts['acc@5']} / {valid_eval} = {acc5_rate:.4f}")

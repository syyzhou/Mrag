...
def evaluate_acc(retrieved, answer_poi_id, all_pois, top_k, per_center_hops):
    """
    计算acc@1和acc@5
    - 只统计下一跳，不统计center POI
    - 每个中心的下一跳先排序（按跳数），然后再处理下一个中心
    
    检索结果顺序：
    - center 1 的 top_per_center_hops 个下一跳
    - center 2 的 top_per_center_hops 个下一跳
    - center 3 的 top_per_center_hops 个下一跳
    - ...
    """
    if answer_poi_id is None:
        return {"acc@1": False, "acc@5": False}

    # 构建有序的候选列表（只包含下一跳）
    candidates = []
    
    # 按center的排名顺序，每个center的下一跳排序后添加
    for item in retrieved[:top_k]:
        next_hops = item["next_hops"]
        # 按跳数排序（次数多的优先）
        sorted_hops = sorted(next_hops.items(), key=lambda x: -x[1])
        for dst_idx, _ in sorted_hops[:per_center_hops]:
            if 0 <= dst_idx < len(all_pois):
                candidates.append(all_pois[dst_idx])
    
    # 计算acc@1和acc@5
    acc1 = candidates[0] == answer_poi_id if candidates else False
    acc5 = answer_poi_id in candidates[:5] if len(candidates) >= 5 else answer_poi_id in candidates
...
def process_qa_file(
    ...
    for row in tqdm(qa_data, desc=f"Retrieving {split_name}"):
        ...
        if answer_poi is not None:
            ...
            # 计算acc@1和acc@5
            accs = evaluate_acc(retrieved, answer_poi, all_pois, top_k=top_k, per_center_hops=per_center_hops)
            if accs["acc@1"]:
                acc_counts["acc@1"] += 1
            if accs["acc@5"]:
                acc_counts["acc@5"] += 1
    ...
    if valid_eval > 0:
        ...
        # 打印acc@1和acc@5（分母为所有样本）
        total = len(qa_data)
        acc1_rate = acc_counts["acc@1"] / total
        acc5_rate = acc_counts["acc@5"] / total
        print(f"  {split_name} acc@1: {acc_counts['acc@1']} / {total} = {acc1_rate:.4f} (valid: {valid_eval})")
        print(f"  {split_name} acc@5: {acc_counts['acc@5']} / {total} = {acc5_rate:.4f} (valid: {valid_eval})")
...
def main():
    ...
    if os.path.exists(args.train_qa):
        ...
        stats = process_qa_file(
            ...
            top_k=args.top_k,
            per_center_hops=args.per_center_hops,
            use_masked=False,  # 检索训练集的全部转移图，不需要mask
            device=args.device,
        )

...
class SemanticEncoder(nn.Module):
    def __init__(
        ...
        self.query_gru = nn.GRU(
            input_size=llm_dim + hour_emb_dim + dow_emb_dim,
            hidden_size=hidden,
            num_layers=1,
            batch_first=True,
        )
    ...
    def encode_query(self, llm_seq, hours, dows, lengths, bucket_ids):
        h_time = torch.cat([self.hour_emb(hours), self.dow_emb(dows)], dim=-1)
        seq_x = torch.cat([llm_seq, h_time], dim=-1)
        ...
        _, h_n = self.query_gru(packed)
        h_query = h_n[-1]
        h_bucket = self.bucket_emb(bucket_ids)
        x = torch.cat([h_query, h_bucket], dim=-1)
...
def build_samples_from_qa(qa_path, p2i, tm, max_query_len=5):
    ...
    for i, item in enumerate(qa_data):
        ...
        query_visits = traj[-max_query_len:] if max_query_len and max_query_len > 0 else traj
        if not query_visits:
            continue

        query_indices = [p2i.get(v["poi_id"], -1) for v in query_visits]
        if any(idx < 0 for idx in query_indices):
            continue
        ...
        samples.append(
            {
                "sample_id": i,
                "query_poi_indices": query_indices,
                "query_hours": [int(v["hour"]) % 24 for v in query_visits],
                "query_dows": [int(v["day_of_week"]) % 7 for v in query_visits],
...
def collate_fn(batch):
    max_len = max(x["query_len"] for x in batch)
    return {
        "query_poi_idx": torch.stack(
            [
                F.pad(torch.tensor(x["query_poi_indices"], dtype=torch.long), (0, max_len - x["query_len"]))
                for x in batch
            ]
        ),
        "query_hours": torch.stack(
            [
                F.pad(torch.tensor(x["query_hours"], dtype=torch.long), (0, max_len - x["query_len"]))
                for x in batch
            ]
        ),
        "query_dows": torch.stack(
            [
                F.pad(torch.tensor(x["query_dows"], dtype=torch.long), (0, max_len - x["query_len"]))
                for x in batch
            ]
        ),
        "query_len": torch.tensor([x["query_len"] for x in batch], dtype=torch.long),
...
def evaluate_coverage(samples, sem_model, sem_all, top_ks, device, count_topk=10):
    ...
    for s in samples:
        ...
        qi = torch.tensor([s["query_poi_indices"]], dtype=torch.long, device=dev)
        qh = torch.tensor([s["query_hours"]], dtype=torch.long, device=dev)
        qd = torch.tensor([s["query_dows"]], dtype=torch.long, device=dev)
        ql = torch.tensor([s["query_len"]], dtype=torch.long, device=dev)
        b = torch.tensor([s["bucket"]], dtype=torch.long, device=dev)

        with torch.no_grad():
            zq = sem_model.encode_query(sem_all[qi], qh, qd, ql, b)
...
def train_semantic(
    ...
    raw_train_samples = build_samples_from_qa(train_qa, p2i, tm, max_query_len=max_query_len)
    train_samples = attach_positives(
        raw_train_samples, fact_index, max_pos=max_pos, drop_zero=True, use_time_mask=True
    )

    print("Building validate samples (using all positives - no time mask)...")
    raw_val_samples = build_samples_from_qa(val_qa, p2i, tm, max_query_len=max_query_len)
    ...
    for ep in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            qhours = batch["query_hours"].to(dev)
            qdows = batch["query_dows"].to(dev)
            qlens = batch["query_len"].to(dev)
            buck = batch["bucket"].to(dev)
            cand = model.encode_candidate(sem_all)
            zq = model.encode_query(sem_all[qidx], qhours, qdows, qlens, buck)
    ...
    config = {
        ...
        "train_qa": train_qa,
        "val_qa": val_qa,
        "train_positive_rule": "time_masked",
...
def main():
    ...
    p.add_argument(
        "--val_qa",
        default="./datasets/nyc/preprocessed/validate_qa_pairs_kqt.json",
    )
    ...
    p.add_argument("--max_query_len", type=int, default=5)

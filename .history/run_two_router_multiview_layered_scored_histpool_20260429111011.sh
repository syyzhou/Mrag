...
TRAIN_FILE="./datasets/nyc/preprocessed/train_qa_pairs_kqt_candidates_target_poi_multiview_dst_time_layered12_far3_mismatch3_tail2_scored_histpool.json"
TEST_FILE="./datasets/nyc/preprocessed/test_qa_pairs_kqt_candidates_target_poi_multiview_dst_time_layered12_far3_mismatch3_tail2_scored_histpool.txt"
OUTPUT_DIR="./output/two-router-target-poi-multiview-dst-time-layered12-far3-mismatch3-tail2-scored-histpool"
SEQ_LEN=4096
DATASET_NAME="nyc"
THRESHOLD_MIB="${THRESHOLD_MIB:-20000}"
POLL_SECONDS="${POLL_SECONDS:-60}"
LOG_DIR="${LOG_DIR:-./logs/run_two_router_multiview_layered_scored_histpool}"
...
wait_for_gpu() {
  ...
  while true; do
    local gpu_info
    gpu_info="$(find_gpu_over_threshold 2>/dev/null || true)"
    if [[ -n "${gpu_info}" ]]; then
      echo "${gpu_info}"
      return 0
    fi

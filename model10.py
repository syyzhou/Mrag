...
class MLPHead(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, config: UmRaConfig,):
        super().__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim, dtype=config.torch_dtype)
        # self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(hidden_dim, num_classes, dtype=config.torch_dtype)
        # self.fc2 = nn.Linear(hidden_dim, num_classes)
        

    def forward(self, x):
        # x = x[:, -1, :]  # 形状: [batch_size, hidden_size]
        x = x.mean(dim=1)  
...
def _apply_hmora(model, config: UmRaConfig) -> PeftModel:
    ...
    # print(config.poi_num)
    # dataset_name = config.dataset.split('/')[2]
    # poi_num = {'nyc': 4981, 'tky': 7833, 'ca': 9690}[dataset_name]
    config.poi_num = 4981
    mlp_head = MLPHead(config.hidden_size, config.hidden_size, config.poi_num, config)
    if hasattr(model, "lm_head"):
        setattr(model, "lm_head", mlp_head)

    trainable_modules = ['router', 'router1', 'router2', 'mora', 'lora', 'task_encoder', 'mlp_head']

...
class MoraModel:
    ...
    def from_pretrained(
        ...
        base_hidden_size = getattr(model.config, "hidden_size", None)
        if base_hidden_size is not None and getattr(config, "hidden_size", None) != base_hidden_size:
            print(
                f"[MoRA-LLaMA] hidden_size mismatch in adapter config: "
                f"{getattr(config, 'hidden_size', None)} -> {base_hidden_size}"
            )
            config.hidden_size = base_hidden_size
        if getattr(config, "use_trajectory_routing", False):
            print(
                f"[MoRA-LLaMA] keeping trajectory_dim={getattr(config, 'trajectory_dim', None)} "
                f"separate from hidden_size={getattr(config, 'hidden_size', None)}"
            )
...
def _align_traj_embedding_dim(traj_embedding: torch.Tensor, target_dim: int) -> torch.Tensor:
    """Align the last dim of trajectory embeddings to target_dim.

    If the incoming embedding is smaller than target_dim, pad zeros on the right.
    If it is larger, truncate on the right.
    """
    current_dim = traj_embedding.shape[-1]
    if current_dim == target_dim:
        return traj_embedding
    if current_dim < target_dim:
        pad_width = target_dim - current_dim
        return F.pad(traj_embedding, (0, pad_width), mode="constant", value=0)
    return traj_embedding[..., :target_dim]
...
class TrajectoryProjector(nn.Module):
    ...
    这里的 traj_dim 指轨迹 pt 文件本身的维度，可以与 LLM hidden_size 不同。
    例如：LLaMA hidden_size=4096，但 trajectory pt dim=2048。
    """
    ...
    def forward(self, traj_embedding: torch.Tensor) -> torch.Tensor:
        expected_dim = self.proj[1].in_features
        actual_dim = traj_embedding.shape[-1]
        if actual_dim != expected_dim:
            print(
                f"[TrajectoryProjector] auto-align input dim: {actual_dim} -> {expected_dim}"
            )
            traj_embedding = _align_traj_embedding_dim(traj_embedding, expected_dim)
...
class FusionGate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, 
        ...
        if traj_embedding is not None:
            ...
            if traj_embedding.shape[-1] != self.traj_dim:
                print(
                    f"[FusionGate] auto-align traj dim: {traj_embedding.shape[-1]} -> {self.traj_dim}"
                )
                traj_embedding = _align_traj_embedding_dim(traj_embedding, self.traj_dim)

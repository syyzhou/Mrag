        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            if 'traj_id' in kqt.keys():
                ...
                if historical_data is not None:
                    ...
                    user_historical_data = historical_data[(str(historical_data['pseudo_session_trajectory_id']) in top200)].tail(100 - num_traj)
                else:
                    # user_historical_data = main_data[(str(main_data['pseudo_session_trajectory_id']) in top200)].tail(200 - num_traj)
                    user_historical_data = main_data[(str(main_data['pseudo_session_trajectory_id']) in top200)].tail(100 - num_traj)
            else:
                if historical_data is not None:
                    user_historical_data = historical_data[(historical_data['UserId'] == user) & (
                            ...
                            historical_data['UTCTimeOffsetEpoch'] < start_time_of_current_traj)].tail(300 - num_traj)
                else:
            ...
            value = {'nyc': 4980, 'tky': 7832, 'ca': 9689}[args.dataset_name]
...
def main():
    ...
    with open(f'{path}train_qa_pairs_kqt_old_traj.json', 'w') as json_file:
        json.dump(qa_dict_train, json_file)


    # Save the test QA pairs in TXT format
    with open(f'{path}test_qa_pairs_kqt_old_traj.txt', 'w') as txt_file:

...
def target_category_key(row, poi_info):
    target = parse_target(row.get("answer", ""))
    return poi_category_key(target, poi_info) if target is not None else None
...
def build_far_target_category(row, last_poi, poi_info, category_index, seen, k, min_km):
    key = target_category_key(row, poi_info)
    return build_far_same_category(last_poi, poi_info, category_index, [key] if key is not None else [], seen, k, min_km)
...
def build_target_category_mismatch(row, last_poi, poi_info, all_pois, seen, k, rng):
    key = target_category_key(row, poi_info)
    return build_category_mismatch([key] if key is not None else [], last_poi, poi_info, all_pois, seen, k, rng)
...
def build_layered(row, raw_idx, retrieval_rows, all_pois, poi_info, category_index, args, use_top12=False):
    ...
    far_candidates = []
    for cand in build_far_target_category(row, last_poi, poi_info, category_index, seen, args.far_same_category_k, args.far_min_km):
        if int(cand["poi_id"]) not in seen:
            far_candidates.append(cand)
            seen.add(int(cand["poi_id"]))

    mismatch_candidates = []
    for cand in build_target_category_mismatch(row, last_poi, poi_info, all_pois, seen, args.category_mismatch_k, rng):
        if int(cand["poi_id"]) not in seen:
...
def main():
    ...
    default_tag = "dst_time_layered12_far3_mismatch3_tail2_targetcat"
    ...
    stats = {
        "rule": (
            ...
            "far/mismatch categories are selected using the ground-truth target POI category; "
            "supplementary candidates randomly inserted while preserving retrieval relative order; "
            "output keeps the original unscored candidate format"
        ),

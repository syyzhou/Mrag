...
case "${DATASET_NAME}" in
  ...
  *)
    ...
    echo "Usage: bash run_single_lora.sh [nyc|tky|ca]"
    exit 1
    ;;
esac
...
OUTPUT_DIR="${OUTPUT_DIR:-./output/single-lora-${DATASET_NAME}}"
SEQ_LEN="${SEQ_LEN:-4096}"
THRESHOLD_MIB="${THRESHOLD_MIB:-25000}"
POLL_SECONDS="${POLL_SECONDS:-60}"
LOG_DIR="${LOG_DIR:-./logs/run_single_lora_original}"

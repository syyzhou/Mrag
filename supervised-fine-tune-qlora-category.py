...
def extract_category_names(text: str) -> List[str]:
    # 匹配形如：category name: xxx.
    return re.findall(r"category name:\s*([^.\n]+)", text)

def find_category_token_indices_by_reconstruction(tokens: List[str], category_names: List[str]) -> List[int]:
    positions = []
    joined_tokens = [t.lstrip("Ġ▁ ") for t in tokens]  # 去掉词边界符号
    start_idx = 0  # 当前起始位置，随匹配推进
    for category in category_names:
        ...
        while i < len(joined_tokens):
            cur = ''
            j = i
            while j < len(joined_tokens) and len(cur) < len(cat_clean):
                cur += joined_tokens[j].lower()
                if cur == cat_clean:
                    positions.extend(range(i, j + 1))
                    start_idx = j + 1  # 下一次从这里继续找
                    break  # 匹配成功，跳出当前 category 的内循环
                j += 1
            if cur == cat_clean:
                break  # 跳出当前 category 的外循环，开始找下一个 category
            i += 1
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, dataset: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        logging.warning("Extracting category token positions...")
        self.data = []
        for i, (ids, label, source_len) in enumerate(zip(data_dict["input_ids"], data_dict["labels"], data_dict["input_ids_lens"])):
            prompt_ids = ids[:source_len]
            tokens = tokenizer.convert_ids_to_tokens(prompt_ids)
            category_names = extract_category_names(sources[i])
            positions = find_category_token_indices_by_reconstruction(tokens, category_names)
            self.data.append({
                "input_ids": ids,
                "labels": label,
                "category_indices": torch.tensor(positions, dtype=torch.long)
            })

        # self.category_token_indices = []
        # for i, (ids, source_len) in enumerate(zip(self.input_ids, data_dict["input_ids_lens"])):
        #     prompt_ids = ids[:source_len]
        #     tokens = tokenizer.convert_ids_to_tokens(prompt_ids)
        #     category_names = extract_category_names(sources[i])
        #     positions = find_category_token_indices_by_reconstruction(tokens, category_names)
        #     self.category_token_indices.append(torch.tensor(positions, dtype=torch.long))  
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        return self.data[i]
        # return dict(input_ids=self.input_ids[i], labels=self.labels[i], category_indices=self.category_token_indices[i])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, category_indices = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "category_indices"))
        ...
        # 融入类别信息
        category_indices = [instance["category_indices"] for instance in instances]
        category_indices = torch.nn.utils.rnn.pad_sequence(
            category_indices, batch_first=True, padding_value=-1
        )
...
def train():
    ...
    # class MyQwen(Qwen2ForCausalLM):
    #     def forward(
    #         self,
    #         input_ids=None,
    #         attention_mask=None,
    #         labels=None,
    #         category_indices=None,  # 你自定义的字段
    #         **kwargs
    #     ):
    #         # 你现在可以使用 category_indices 做一些事情，比如：提取 embedding 等

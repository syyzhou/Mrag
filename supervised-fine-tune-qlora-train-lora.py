...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lambda_weight: float = field(
        default=0.5,
        metadata={"help": "The weight of semantic contrastive loss."}
    )
    gamma: float = field(
        default=0.3,
        metadata={"help": "The gamma value for contrastive loss."}
    )
...
class UmraTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        """
        自定义初始化，添加 lambda_weight 和 neg_weight 参数
        """
        super().__init__(*args, **kwargs)  # 调用父类的 __init__
        self.tokenizer = kwargs.get('tokenizer', None)
        # self.train_dataset = kwargs.get('data_module', None)['train_dataset']
        data_path = f'./datasets/nyc/preprocessed/'
        self.indexer = POIIndexer(gpu_id=3)  # 选择GPU 3
        index_file_path = data_path + "poi_desc_index_cpu.faiss"  # 注意：使用CPU索引文件名
        mapping_file_path = data_path + "poi_desc_with_id.csv"
        print("索引文件已存在，加载索引...")
        self.indexer.load_index(index_file_path, mapping_file_path)
    
    def compute_loss(self, model, inputs, return_outputs=False):
        outputs = model(**inputs)
        # loss = outputs.loss
        loss_gen = outputs.loss   # 原始生成损失 (交叉熵)
        attention_mask = inputs.get("attention_mask", None)
        
        # 添加自定义的 auxiliary loss
        if hasattr(model, "router_manager") and hasattr(model.router_manager, "get_auxiliary_loss"):
            loss_gen = model.router_manager.get_auxiliary_loss(loss_gen, attention_mask)

        labels = inputs['labels']
        logits = outputs.logits  # logits shape: [batch_size, seq_length, vocab_size]
        predicted_ids = torch.argmax(logits, dim=-1)
        generated_ids = []
        source_lens = [(label == IGNORE_INDEX).nonzero(as_tuple=True)[0].size(0) for label in labels]
        for idx, length in enumerate(source_lens):
            generated_ids.append(predicted_ids[idx, length:])  # 从有效长度位置开始的部分作为生成部分
        pre = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in generated_ids]
        target_desc = [self.tokenizer.decode(label[label != IGNORE_INDEX].tolist(), skip_special_tokens=True) for label in labels]
        
        def extract_description(text):
            return re.sub(r'.*<POI description>(.*?)</POI description>.*', r'\1', text)
        # 获取描述信息
        target = list(map(extract_description, target_desc))
        predictions = [match.group(1) if (match := re.search(r'description>(.*?)</', text)) else text for text in pre]
       
        v_gen = torch.tensor(self.indexer.encode(predictions))  # 将numpy数组转换为PyTorch Tensor
        target_embeddings = torch.tensor(self.indexer.encode(target))  # 同样转换target_embeddings
        sim_pos = F.cosine_similarity(v_gen, target_embeddings, dim=-1)  # 计算余弦相似度
        
        # 负样本计算
        sim_neg = self.get_negative_loss(predictions, v_gen, target_desc) 
        
        # 计算对比损失：批量计算所有样本的对比损失
        contrastive_loss = torch.max(self.args.gamma + sim_neg - sim_pos, torch.tensor(0.0))  # 计算每个样本的对比损失
        loss_contrast = contrastive_loss.mean()  # 对所有样本的对比损失取平均

        # 计算总损失
        loss_total = loss_gen + self.args.lambda_weight * loss_contrast
        
        print(f"loss_gen: {loss_gen.item():.4f}, loss_contrast: {loss_contrast.item():.4f}, loss:{loss_total.item()}" )
        print(f"contrast_ratio: {(loss_contrast/loss_gen).item():.4f}")
        return (loss_total, outputs) if return_outputs else loss_total
    
    def get_negative_loss(self, predictions, v_gen, target_desc):
        # 批量检索：检索每个样本的前 2 个最相关的 POI
        D, I = self.indexer.index_gpu.search(np.array(v_gen).astype('float32'), k=2)  # 批量检索前 2 个最相关的 POI
        negative_similarities = []

        for i in range(len(predictions)):
            retrieved_poi = self.indexer.poi_mapping.iloc[I[i][0]]  # 获取第一个最相似的 POI
            gt_poi_id = find_poi_id_from_gt(target_desc[i])  # 获取真实 POI ID
            
            # 判断检索到的 POI 是否与 ground truth 匹配
            if str(retrieved_poi['poi_id']) == str(gt_poi_id):  # 正样本
                is_correct = True
            else:  # 负样本
                is_correct = False

            # 负样本选择
            if is_correct:  # 如果检索到的就是正确的
                similarity = D[i][1]  # 第二个相似的
            else:
                # 如果检索到的 POI 和目标 POI 不匹配，那么直接认为检索到的 POI 就是负样本
                similarity = D[i][0]  # 第一个相似的距离

            negative_similarities.append(similarity)

        negative_similarities = torch.tensor(negative_similarities)  # 转换为张量
...
def train():
    ...
    umra_config.torch_dtype = torch.bfloat16
    umra_config.padding_side = tokenizer.padding_side
    ...
    lora_config = LoraConfig(
        ...
        lora_alpha=16,
        target_modules=[
            f"model.layers.{i}.self_attn.q_proj.lora" for i in range(28)  
            ] + [
            f"model.layers.{i}.self_attn.k_proj.lora" for i in range(28)  
            ] + [
            f"model.layers.{i}.self_attn.v_proj.lora" for i in range(28)  
            ] + [
            f"model.layers.{i}.self_attn.o_proj.lora" for i in range(28) 
        ],
        lora_dropout=0.1,
        bias="none",
        inference_mode=False,
    )
    model = get_peft_model(model, lora_config, adapter_name="Desc_lora")
    ...
    # 合并配置的函数
    # def merge_configs(model_config, lora_config):
    #     # 这里可以根据实际需要合并两个配置
    #     merged_config = model_config.export()  # 基础模型的配置
    #     lora_config_dict = lora_config  # LoRA 配置
    #     merged_config.update(lora_config_dict)  # 合并
    #     return merged_config
    # merged_config = merge_configs(model.peft_config, peft_config)
    # print(merged_config)
    # setattr(model, 'peft_config', merged_config)

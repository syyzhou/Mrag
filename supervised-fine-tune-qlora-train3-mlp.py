...
from model10 import MoraModel, get_peft_model
import re
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output2_dir: str = field(default="./output/umra_train1/")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, dataset: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        for example in list_data_dict:
            ...
            if ": " in answer_text:
                numeric_part = answer_text.split(": ")[1]
                self.labels.append(int(numeric_part))
            else:
                self.labels.append(int(answer_text))  # 回退到原始方式
...
def train():
    ...
    poi_num = {'nyc': 4982, 'tky': 7834, 'ca': 9691}[dataset_name]
